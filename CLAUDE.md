...
- **Chatwork も担当者ごとにルームを割り当てられる**（issue #1 追記分）。Slack と同じで**API トークンはテナント共通のものを使い回し、差し替えるのはルーム ID だけ**（担当者ごとにトークンを発行させない）。送信の実体は `services/chatwork.py` に集約し、宛先の組み立ては `staff_routing.chatwork_send_rooms()`（共通ルームと同じ ID なら二重投稿しない）。**トークン未設定なら何も送らない**。ルーム ID だけ指定して保存しようとすると API が 400 で拒む（「設定したのに届かない」を作らない）。
- **意図的な挙動変更**: **受付通知が Chatwork にも流れるようになった**。従来 Chatwork は `api/kiosk.py` 直書きで**配達の呼び出しだけ**が使っており、管理画面「通知設定」の Chatwork カードは来客通知の宛先のつもりで設定していても届かなかった（UIと実装の食い違い）。共通ルームへ送るのは `include_default` が ON のときだけ。
- **Web Push は担当者ごとに割れる**（issue #1 追記分）。購読(`push_subscriptions`)は端末ではなく**ログインした管理ユーザー**に紐づくので、`staff_notification_routes.push_user_id` で「この担当者あては誰の端末へ」を結びつける。`reception_notify.push_targets()` が宛先を決め、**同じ endpoint へは1回だけ**送る。
- **`include_default`（既定 ON・担当者ごとにOFF可）**でテナント共通の通知先へも送るかを決める。OFF かつ `push_user_id` 未設定なら Web Push は送らない（従来どおり）。**代理通知で宛先が絞れないときは `include_default` に関わらず全購読へ送る**（誰も応答していない状態で誰にも届かないのが一番危ないので安全側へ倒す）。代理担当者にプッシュ先ユーザーが設定されていればその人だけに送る。代理通知のプッシュは `tag` を `reception-{id}-esc` にして元の通知を上書きしない。
...
- **担当者マスターの編集は「通知設定」に集約**（issue #1 追記分）。実体は `tenants.staff_list`（カンマ区切り）のままで、**キオスク設定（受付設定）側は読み取り専用**にして誘導リンクだけ置く。宛先を決める場所と担当者を足す場所が別画面なのは分かりにくいため。API は `PUT /notifications/staff-routes/staff`（リストを丸ごと置換＝追加・削除・並べ替え）と `POST /notifications/staff-routes/staff/rename`。**名前にカンマは使えない**（保存形式が壊れるので 422）。
  ...
  - **改名は通知先設定・代理通知先・まだ応答していない受付(`state='received'`)まで追随させる**。進行中の受付を旧名のまま残すと、その受付の代理通知が旧名で設定を探して見つけられず、**安全網が黙って外れる**。**過去の完了ログは当時の記録なので書き換えない**。
