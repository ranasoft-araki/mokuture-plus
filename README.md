...
- **`start` は停止の逆順（音声 → 本体 → ブラウザ）。** 受付画面は起動時に一度だけ
  `/voice/status` を見て、届かなければそのページでは「音声で入力」を出さない
  （`ensureVoiceStatus` が `VOICE.checked` で latch する）。本体を先に上げると
  ブラウザが数秒で開き、起動中の音声サービスに間に合わずボタンが消える。
