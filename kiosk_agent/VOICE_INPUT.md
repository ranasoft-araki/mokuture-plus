...
なお、**読み仮名の登録と Vosk の発音辞書は別物**。読みをここに書いても、モデルの
発音辞書（`graph/words.txt`、20万語）に無い表記の担当者は §4-7 の語彙を絞る 2 パス目に
入れられない（従来どおり 1 パス目の読み照合だけが頼りになる）。入れられなかった名前は
ログに出る:

```
journalctl -u mokuture-voice | grep "発音辞書に無いため"
```
...
### 4-7. 担当者の語彙で decode し直す（2 パス目）

一般語の言語モデルは固有名詞に弱い。クリーン音源10本の実測で「服部様」が
「酉様」「都立様」「服部祖」に化けた。**辞書落ちではない**（20万語の発音辞書に
「服部」「磯野」「荒木」はある）。文脈で別の語に負けている。

担当者は「誰がいるか」が分かっているので、その語彙だけに絞って decode し直す。
キオスクが録音開始時に渡す担当者一覧（`ListenBody.staff`）をそのまま使うので、
画面側の変更は要らない。

| | 担当者の特定 | 誤って別人を候補に出した数 |
|---|---|---|
| 1 パス目だけ（従来） | 8/10 | 0 |
| 2 パス目込み | **10/10** | 0 |

救われるのは**読みごと失われた**場合（「酉様」「都立様」）。読みが残っていれば
（「服部祖」）1 パス目の読み照合で当たるので、差はそこだけ出る。

歯止め:

- 2 パス目は**担当者の候補にだけ**使う。会社名・氏名には使わない（絞った語彙は
  来訪者の会社名を知らないので、そちらは 1 パス目の方が当たる）
- 呼ばれた担当者が名簿に無いと、別人の名前を埋めることがある（実測 10件中 2件）。
  ただし誤って埋めた語は信頼度が落ちる（実測 0.657〜0.871 / 正解は 6/6 すべて 1.000）
  ので `vosk.grammar_min_conf`（既定 0.9）で捨てる。捨てたら候補なし＝画面で選ぶ
  従来の動きに戻るだけで、**間違った人は呼ばない**
- 2 パス目にも「名乗りの範囲を除く」処理をかける。除かないと
  「山田運送の田中です」の田中が、今度は信頼度 1.0 付きで担当者に化ける
- `[unk]` は語彙外の音の逃げ場。**欠落区間として扱い、前後を連結しない**
  （連結すると偽の名前ができる）
...
認識器はフリー用と語彙限定用を別に持つ（作り直しは重いので、担当者一覧が
変わったときだけ作り直す）。**サービス起動後の最初の 1 人だけ**、2 パス目の
認識器を組む分だけ余分に待つ。時間は `scripts/voice_bench.py` で測ること。

### 4-8. クラウド ASR との比較（実測・2026-09-23）

飛び込み来訪者の会社名はローカルでは取れない。候補が無いので当てようがなく、これは
モデルの大きさの問題ではない（1GB の `vosk-model-ja-0.22` でも会社名 4/10 と改善せず、
しかも `Gr.fst` が無いので語彙を絞る手が使えなくなる）。そこで AmiVoice API を同じ音源
10 本・同じ採点（`scripts/voice_eval.py` の `score_case`）で測った。

| | 会社名 | 氏名 | 訪問先が候補に出た | 用件 | アポ |
|---|---|---|---|---|---|
| ローカル Vosk（2パス） | 5/10 | 8/10 | 10/10 | 10/10 | 10/10 |
| AmiVoice 素 | **9/10** | **10/10** | 10/10 | 10/10 | 10/10 |
| AmiVoice ＋単語登録 | 9/10 | 10/10 | 10/10 | 10/10 | 10/10 |

```
standard  ローカル  : 私その木工所の荒木と申します本日服部様との打ち合わせの約束でまいりました
          AmiVoice : 私磯野木工所の荒木と申します本日服部様と打ち合わせの約束でまいりました
no_host   ローカル  : その報告書の荒木です打ち合わせで気もした
          AmiVoice : 磯野木工所の荒木です。打ち合わせできました。
```

分かったこと:

- **会社名 5/10 → 9/10。** ローカルで解けなかった未知の固有名詞が取れる。トークンには
  読みも付く（`{"written":"磯野","spoken":"いその","confidence":1.00}`）ので、読みで
  照合する既存の方針とも噛み合う
- **単語登録（`profileWords`）は差を生まなかった**（素と同じ 9/10）。担当者は素で取れて
  いるため。**ローカルとは事情が逆で、クラウドでは候補を渡さなくても当たる**
- 外したのは `reordered`（語順が崩れた発話）だけ: 「その交渉の荒木です」
- 往復は中央値 1.1 秒（開発機から）。受付で待てる 3 秒には収まるが、**ネットワーク次第**
- 費用は 20 回で 11.25 円。受付 1 人 10 秒・50人/日・22日で月 484 円/拠点（ログなし
  158.4円/時間）。**コストは判断材料にならない**

残る判断は精度ではなく要件。音声は来訪者の氏名・所属・訪問先で、これを外部へ出すかどうか。
出さない前提を崩さないなら、飛び込みの会社名は名刺読み取りに倒すのが筋（[CARD_READER.md](CARD_READER.md)）。

測り直す手順は `scripts/amivoice_eval.py` の冒頭に書いてある。**この音源は架空の名乗りで、
実在の個人・法人を含まない。** 実機の肉声で測ると結果は変わりうる（マイク・騒音）。

### 4-9. 担当者が自由入力の端末

担当者を名簿（管理画面の受付設定→担当者一覧）から選ばせず、**自由入力**にしている
端末がある。その場合「名簿にいる人しか訪問先になり得ない」という前提が成り立たず、
「服部様とお約束で参りました」と正しく聞き取れていても訪問先が空のままになる
（実機で発覚）。

そこで**名簿が空のときだけ**、敬称の直前を呼び名として拾う。

```
磯野木工所の荒木と申します服部様とお約束でまいりました
  → 会社名 磯野木工所 / お名前 荒木 / 訪問先 服部
```

- 「お客様」「担当の方」のように敬称が付いても人名でないものは拾わない
- 名乗りの範囲（「山田運送の田中です」）は元から除いてあるので、来訪者の名前が
  訪問先になることはない
- **名簿があるときは絶対にやらない。** 名簿にいない名前を候補に混ぜると、実在しない
  担当者を選べてしまい、通知の宛先も決まらない

画面側の変更は要らない。訪問先の欄は名簿が空のとき自由入力になり、候補が1つあれば
それが初期値に入る作りになっている。

### 4-10. クラウド音声認識への中継（鍵を端末に置かない）

4-8 の実測でクラウドの方が固有名詞に強いと分かったが、**発売済みの端末すべてに
APPKEY を配って回るのは現実的でない**（端末が持ち出されたときに止められない、という
問題もある）。そこで中継にする。

```
キオスク(音声サービス) ──音声──> 自社API /kiosk/voice/transcribe ──> AmiVoice
...
待ち、間に合えばクラウドの文字起こしを採り、間に合わなければローカルの結果をそのまま
使う。**通信が遅くても待ち時間は増えない。**

担当者の候補は、クラウドを採ったときもローカルの2パス目（4-7）の結果を併せて使う。

端末側の元栓は `voice_input.yaml`:

```yaml
cloud:
  enabled: true          # false にするとサーバの設定に関係なく問い合わせない
  timeout_sec: 6.0
  wait_after_local_sec: 1.8
```

状態は `/voice/status` の `cloud` に出る。`available:false` の `detail` に理由が入る
（未登録・設定で無効・サーバ側が無効）。

**音声も認識結果も保存しない。** AmiVoice 側もログを残さないエンドポイントを使い、
サーバのログに出るのは大きさと所要時間だけ。ただし**来訪者の氏名・所属・訪問先が
外部へ出ることに変わりはない**ので、有効にするかはテナントごとの契約・同意の話。
...
| 担当者が当たらない・別人が候補に出る | `journalctl -u mokuture-voice` の「担当者パス」の行。候補 0 なら信頼度で落ちている（§4-7）。`staff_readings.yaml` が例のままでないかも確認する |
| 会社名・氏名が見当はずれ | 予定に無い来訪者の会社名は語彙に無いので、1 パス目の当てずっぽうになる。確認画面で直す前提（§4-7） |
