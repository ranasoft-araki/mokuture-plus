...
def main() -> int:
    print(f"{'pattern':22} {'det':>4} {'iou':>6} {'area':>6} {'asp':>5} {'txt':>4} {'ms':>6}  expect")
    bad = 0
    for name in mf.PATTERNS:
        ...
        if det is None:
            got, iou, area, asp, txt = "no", 0.0, 0.0, 0.0, 0
        else:
            got = "yes"
        ...
        if not ok:
            bad += 1
        print(f"{name:22} {got:>4} {iou:6.3f} {area:6.3f} {asp:5.2f} {txt:4d} {ms:6.1f}  {expect} {'' if ok else '  <-- NG'}")
