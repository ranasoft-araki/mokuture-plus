...
DEFAULTS: dict[str, Any] = {
    ...
    # クラウド音声認識への中継(自社バックエンド経由 / voice/cloud.py)。
    # **鍵は端末に置かない。** 実際に使えるかはサーバが決める(サーバに鍵があり、かつ
    # テナントが許可しているときだけ)。使えなければローカル認識だけで従来どおり動く。
    "cloud": {
        # 端末側の元栓。false ならサーバの設定に関係なく問い合わせない。
        "enabled": True,
        # 自社 API の入口。空なら本体の .env の REMOTE_API_URL を使う。
        "api_url": "",
        # 1 回の往復に許す時間。
        "timeout_sec": 6.0,
        # **ローカルの認識が終わってから、クラウドを待つ上限。** ローカルは先に
        # 終わるので、ここを超えたらローカルの結果をそのまま使う = 待ち時間が
        # 増えない。受付で待てるのは全体で 3 秒(voice_bench.py の TARGET_SEC)。
