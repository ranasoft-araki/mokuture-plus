...
優先順位は後勝ち:
  1. voice/defaults.py の DEFAULTS
  2. kiosk_agent/voice_input.yaml (任意。PyYAML が無ければ黙って飛ばす)
  3. 環境変数 VOICE_<SECTION>__<KEY>(例: VOICE_WHISPER__MODEL_PATH, VOICE_AUDIO__DEVICE)

値の型は既定値の型に合わせて変換する(既定が float なら float、list なら
カンマ区切りを分解)。既定に無いキーは無視する(打ち間違いで壊れないように)。

名刺読み取りの `card/settings.py` と同じ作法。読む場所と接頭辞だけが違う。
"""
...
from voice.defaults import DEFAULTS

log = logging.getLogger(__name__)

AGENT_DIR = Path(__file__).resolve().parent.parent
CONFIG_PATH = Path(os.environ.get("VOICE_CONFIG", str(AGENT_DIR / "voice_input.yaml")))
...
def _apply_env(cfg: dict) -> None:
    """VOICE_A__B__C=値 の形の環境変数を反映する。"""
    for env_key, raw in os.environ.items():
        if not env_key.startswith("VOICE_") or env_key == "VOICE_CONFIG":
            continue
        parts = [p.lower() for p in env_key[len("VOICE_"):].split("__") if p]
        if not parts:
            continue
        node: Any = cfg
        for p in parts[:-1]:
...
def _load() -> dict[str, Any]:
    ...
    if CONFIG_PATH.exists():
        try:
            ...
            _load_notes.append("PyYAML not installed; voice_input.yaml skipped")
        else:
            try:
                ...
                if isinstance(data, dict):
                    ...
                    _load_notes.append("voice_input.yaml is not a mapping; ignored")
            except Exception as e:      # 壊れた YAML で音声サービスを止めない
                _load_notes.append(f"voice_input.yaml parse failed ({type(e).__name__}); using defaults")

    _apply_env(cfg)
    for note in _load_notes:
        log.info("[voice] config: %s", note)
    return cfg
...
def get(path: str, default: Any = None) -> Any:
    """ドット区切りで取り出す。例: get("vad.silence_sec")"""
    node: Any = cfg()
