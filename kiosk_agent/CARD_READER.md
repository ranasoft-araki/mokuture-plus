...
肌色の範囲は `detection.skin_cr_min` ほかで調整できる。`skin_luma_max` は
「生成りの紙を肌と間違えない」ための明るさの上限で、上げすぎると白い名刺が
肌と判定されて `max_skin_ratio` で落ちるようになる。
...
| 白い名刺が肌と判定されて落ちる | `detection.skin_luma_max` | 下げる（225 → 200） |
