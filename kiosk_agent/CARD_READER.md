...
`out_of_frame` と占有率をそのままでは使えない（外へ広げた矩形は余白ぶん必ず
画面の端に当たる）。代わりに

* 見切れ: **文字そのもの**が画面の端に達していたら `out_of_frame`。読めない文字が
  ある状態で撮影して項目を落とさないため
* 大きさ: 「字が読める大きさか」（`quality.text_height_min`）

**残っている限界。** 名刺と同じ縦横比の印刷物（封筒など）は、縁からも文字からも
名刺と区別できない。撮影しても受理判定（氏名か社名が取れていること）で弾かれ、
撮り直しになる。
