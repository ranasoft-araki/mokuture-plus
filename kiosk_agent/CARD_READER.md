...
- [実機で読めないときの調べ方（フレーム保存）](#実機で読めないときの調べ方フレーム保存)
- [API 仕様](#api-仕様)
- [処理時間の計測方法](#処理時間の計測方法)
...
| | 検出率 | 撮影に進めたフレーム | 自動撮影 | 読めた回数 |
|---|---|---|---|---|
| 縁だけ | 27.2% | 251 | 15 回 | 14 回 |
| **文字の並びも使う（既定）** | **81.3%** | **746** | **31 回** | **23 回** |

**誤検出を止める仕組み。** 文字だけを見ると、A4 の書類・スマートフォンの画面・
レシートを名刺と取り違える。3 段で止めている。

1. 文字が入る範囲の縦横比に上限を置く（`detection.text_max_region_aspect`）。
   名刺は正方形に近いことも横長なこともあるが、レシートのような帯にはならない
2. 「名刺の形ではないが大きくて四隅が直角に近い四角形」が写っていて、**拾った
   文字がその四角形の上にある**なら名刺と見ない（`detection.text_veto_*`）。
   一律に止める作りにすると、縁が壊れているだけの本物の名刺まで落ちた
3. 撮影可否の判定（下記）

2 の裏付け（辺に本当の段差があるか）は、文字から候補が出て、かつその四角形と
重なっているときにだけ測る。毎フレーム測ると 1 フレームの処理が 3 割伸びる。

**撮影可否の判定が変わる。** 文字から決めた四隅は名刺の縁ではないので、
`out_of_frame` と占有率をそのままでは使えない。

* 大きさは「字が読める大きさか」で見る（`quality.text_height_min_ratio`）。
  **しきい値は画面幅に対する比で持つこと。** OCR がかかるのは検出フレーム(640px)
  ではなく撮影画像(最大 2048px)なので、読めるかどうかは撮影画像上の字の高さで
  決まる。検出フレームの画素数で決め打ちにしたところ、実機で「名刺が画面から
  はみ出すほど近づけないと認識しない」状態になった。実測（撮影解像度を落として
  測定）では撮影画像上 6px あたりまで 4〜9 項目取れ、4px で 2 項目まで落ちる。
  既定 0.0055（= 640px 幅で 3.5px）なら名刺が画面幅の 3 割ほどで通る
* **見切れでは止めない。** 縁が見えない以上「名刺が切れている」のか「画面の端に
  たまたま文字（ブラインドの桟・襟など）がある」のかを区別できない。止める作りに
  したところ、実機の映像では後者で止まって**まったく読み取れなくなった**。誤って
  止めると利用者は何もできないが、誤って撮っても受理判定（氏名か社名が取れている
  こと）で弾かれて撮り直しになるだけで、損得が釣り合わない。見切れていること自体は
  記録だけ残す（`FrameMetrics.text_clipped`）

**文字の取り込みは「いちばん近い文字から」測る。** かたまりの外接矩形から測ると、
端の 1 文字を取り込むたびに矩形が広がり、次のまわりがまた広がる…と連鎖して
画面全体に届く。実機の映像で実際にそうなり、領域が画面いっぱいになって
撮影できなくなった。

**残っている限界。**

* 名刺と同じ縦横比の印刷物（封筒など）は、縁からも文字からも名刺と区別できない。
  宛名に会社名が入っていると受理判定も通ってしまう。利用者が確認画面で気付ける
  ことを前提にしている
* 名刺が画面から見切れていても撮影に進む（上記のとおり意図的）。読めた範囲だけが
  確認画面に出るので、足りない項目は手入力してもらう
* A4 の書類は撮影に進むことがあるが、本文からは氏名も社名も取れないため受理判定で
  弾かれ、撮り直しになる（実測 信頼度 0.19）
...
| 遠いのに撮影に進んでしまう | `quality.text_height_min_ratio` | 上げる（0.0055 → 0.009） |
| 近づけないと認識しない | `quality.text_height_min_ratio` | 下げる（0.0055 → 0.004） |
...
## 実機で読めないときの調べ方（フレーム保存）

**既定では無効。設定を書かない限り 1 バイトも書き出さない。**

読み取れない原因を追うとき、画面録画では足りない。録画に写っているのは縮小された
プレビューを再エンコードしたものなので、**検出範囲が当たっているかは分かっても
OCR の精度は測れない**。しきい値を直すには、端末が実際に OCR へ渡している画像が要る。

そのためだけの逃げ道。**書き出すのは個人情報そのもの**なので、調べ終わったら必ず消す。

### 有効にする

```bash
# 一時的に（このプロセスだけ）
CARD_DEBUG__DUMP_DIR=card_dump uv run uvicorn main:app --host 0.0.0.0 --port 8080

# systemd で動かしている端末なら card_reader.yaml に書く
debug:
  dump_dir: card_dump        # 空 = 無効（既定）
  dump_max_captures: 40      # 超えたら古い撮影から消す
```

有効な間は、**保存のたびに警告ログが出る**（入れっぱなしに気づけるように）。
`/card/status` の `debug_dump_dir` にも出る。

### 書き出されるもの（撮影 1 回につき 4 ファイル）

| ファイル | 中身 |
|---|---|
| `NNN_capture.jpg` | ブラウザから届いた撮影画像そのもの。検出はこれに対して行う |
| `NNN_region.jpg` | 上の画像に検出した四隅を描いたもの。**範囲の当たり外れが一目で分かる** |
...
```json
"detection": {
  "source": "text",          // edge=紙の縁 / text=文字の並び
  "cover_ratio": 0.44,       // 外接矩形が画面に占める割合。背景まで掴むと 1 に近づく
  "text_regions": 112,       // 名刺なら 30-60 程度。多すぎる＝背景を拾っている
  "text_height_px": 6.0      // 撮影画像上の字の高さ
}
```

`cover_ratio` が大きく `text_regions` が名刺の実勢より多いときは、背景（横桟の
ブラインド・棚・扉枠）を文字として取り込んでいる。`NNN_region.jpg` を見れば確認できる。

### 調べ終わったら消す

```bash
rm -rf kiosk_agent/card_dump
```

件数の上限（`dump_max_captures`）を超えると古いものから自動で消えるが、
**放置しない**こと。設定を消せば以後は 1 バイトも書かない。
...
`card_half_out`（名刺が画面から大きくはみ出している）/ `receipt`（レシート）/
