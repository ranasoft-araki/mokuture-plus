...
- 縦書きの名刺は苦手（行の検出はできるが、認識精度は横書きより落ちる）
- 手書きの名刺は読めない
...
■ この写真の実測値（card_reader.yaml のしきい値と見比べる）
  ...
  面積比            0.392   （quality.capture_area_min = 0.12）
```
...
`with_kana`（ふりがな付き）/ `not_a_card_paper`（名刺でない紙）/
`not_a_card_phone`（スマートフォン画面）/ `empty_desk`（何も無い）
...
- 面積比が `capture_area_min`（0.12）より小さい → 名刺をカメラに近づける
...
エッジ抽出を 3 通り用意し、名刺候補が採れた時点で打ち切っている。
