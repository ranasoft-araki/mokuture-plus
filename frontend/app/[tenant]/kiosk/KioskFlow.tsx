...
// ─── Types ────────────────────────────────────────────────────────────────────
type KioskScreen = "idle" | "top" | "reception" | "qr" | "calling" | "complete";
...
const VALID_SCREENS: KioskScreen[] = ["idle", "top", "reception", "qr", "calling", "complete"];
...
function TopScreen({
  ...
  const tiles = [
    ...
    { Icon: QRIcon,        label: "QR で受付",   sub: "招待コードをお持ちの方",               action: () => onNavigate("qr") },
...
function QrScreen({
  ...
  const handleQRData = async (raw: string) => {
    ...
    try {
      ...
      stopCamera();
      onComplete({ name, company, staff: "", purpose });
    } catch (err: unknown) {
  ...
  return (
    <KioskScaler bg="#faf8f4">
      <div
        ...
        onClick={resetIdle}
      >
        <div style={{ flex: 1, padding: "24px 80px 0", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "center", minHeight: 0 }}>
          {/* Left: camera */}
          <div style={{ background: "#1d1a15", borderRadius: 28, position: "relative", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", height: "100%", maxHeight: 760 }}>
            {submitting ? (
              ...
              <>
                <div style={{ position: "absolute", inset: 60, border: "3px dashed rgba(255,255,255,0.2)", borderRadius: 20, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(255,255,255,0.02)" }}>
                  {[{ top: 0, left: 0, rot: 0 }, { top: 0, right: 0, rot: 90 }, { bottom: 0, right: 0, rot: 180 }, { bottom: 0, left: 0, rot: 270 }].map((c, i) => (
                    <div key={i} style={{ position: "absolute", ...(c.top !== undefined ? { top: -3 } : { bottom: -3 }), ...(c.left !== undefined ? { left: -3 } : { right: -3 }), width: 56, height: 56, transform: `rotate(${c.rot}deg)`, borderTop: "5px solid #4a7c4e", borderLeft: "5px solid #4a7c4e", borderRadius: "12px 0 0 0" }} />
                  ))}
                  {scanning && (
                ...
                <video ref={videoRef} muted playsInline style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", opacity: 0.85 }} />
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            ...
            {error && (
              <div style={{ padding: "16px 20px", background: "#f6e0dc", border: "1px solid rgba(168,66,56,0.3)", borderRadius: 14, color: "#a84238", fontSize: 16, lineHeight: 1.6, marginBottom: 24 }}>{error}</div>
            )}
    ...
    <div key={screen} style={{ animation: "kiosk-fade-in 0.18s ease-out" }}>
      ...
      {screen === "qr" && (
        <QrScreen
          settings={settings}
          deviceName={deviceName}
          onComplete={(data) => { setFlowData(prev => ({ ...prev, ...data })); setScreen("calling"); }}
          onBack={() => setScreen("top")}
          onForm={() => setScreen("reception")}
          onIdle={() => setScreen("idle")}
        />
      )}
      {screen === "calling" && (
